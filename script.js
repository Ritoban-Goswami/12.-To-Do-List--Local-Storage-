...
const loadTasks = () => {
    if (localStorage.getItem('tasks') === null) {
        ...
        tasksArr.forEach(storageElem => {
            list.innerHTML += `<li class="tasks py-3"><div class="d-flex justify-content-between align-items-center">
            ${storageElem}<i class="fa-solid fa-xmark close" onclick ="clrEach(event)"></div></i>
    </li>`
        })
    }
}
...
const addTask = () => {
    ...
    if (inputValue === '') {
        ...
        list.innerHTML += `<li class="tasks py-3"><div class="d-flex justify-content-between align-items-center">
        ${inputValue}<i class="fa-solid fa-xmark close" onclick ="clrEach(event)"></div></i>
    </li>`
        input.value = ''
    }
}
